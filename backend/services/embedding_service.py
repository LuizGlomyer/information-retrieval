...
class EmbeddingService:
    ...
    def _load_model(self) -> None:
        ...
        if self._model is None:
            print(f"\n🔄 Loading embedding model: {config.BM25_EMBEDDING_MODEL_NAME}...")
            self._model = SentenceTransformer(config.BM25_EMBEDDING_MODEL_NAME)
            ...
            print(
                f"✓ Model loaded successfully (dimension: {config.BM25_EMBEDDING_DIMENSION}, device: {self._device.type})"
            )
